...
export default function TechnicalAnalysisPage() {
  ...
  const [items, setItems] = useState<TechnicalAnalysisTagDto[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [showDialog, setShowDialog] = useState(false);
  const [editItem, setEditItem] = useState<TechnicalAnalysisTagDto | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<TechnicalAnalysisTagDto | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      attachToken();
      const res = await getTechnicalAnalysisTags();
  ...
  const openEdit = (item: TechnicalAnalysisTagDto) => {
    setEditItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      attachToken();
      if (editItem) {
        await updateTechnicalAnalysis({ id: editItem.id, ...form });
        toast({ title: "Updated", description: `"${form.name}" updated.` });
      } else {
        await createTechnicalAnalysis(form);
  ...
  const handleDelete = async () => {
    ...
    try {
      attachToken();
      await deleteTechnicalAnalysis(deleteTarget.id);
  ...
  const filtered = items.filter(
    (i) =>
      ...
      (i.shortName && i.shortName.toLowerCase().includes(search.toLowerCase()))
  );

  return (
    <div className="space-y-4">
      <SectionHeader
        title="Technical Analysis Tags"
        description="Indicators and patterns used in technical analysis."
        count={items.length}
        onAdd={openCreate}
        searchValue={search}
      ...
      {loading ? (
        ...
        <EmptyState title="No technical analysis tags yet" action={openCreate} />
      ) : (
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/30 hover:bg-muted/30">
                ...
                <TableHead className="text-xs uppercase tracking-wider">Short Name</TableHead>
            ...
            <TableBody>
              {filtered.map((item) => (
                <TableRow key={item.id} className="group">
                  ...
                  <TableCell>
                    {item.shortName ? (
                      <Badge variant="secondary" className="text-xs">{item.shortName}</Badge>
                    ) : (
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editItem ? "Edit" : "New"} Technical Analysis Tag</DialogTitle>
            <DialogDescription>{editItem ? "Update the tag details." : "Add a new indicator or pattern."}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">Name *</label>
              <Input placeholder="e.g. Moving Average" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">Short Name</label>
              <Input placeholder="e.g. MA" value={form.shortName} onChange={(e) => setForm({ ...form, shortName: e.target.value })} />
            </div>
            <div className="space-y-2">
