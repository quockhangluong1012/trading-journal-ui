  ...
  return (
    <div className="flex flex-col h-full bg-card text-[13px] relative font-sans w-full">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-border/50">
        ...
        <div className="flex items-center gap-0.5 text-muted-foreground mr-[-8px]">
          <Button
            variant="ghost"
            size="icon"
            className="h-[28px] w-[28px] rounded-[4px] hover:text-foreground"
          >
      ...
      <div className="flex px-4 py-2.5 gap-1 border-b border-border/50">
        <div className="flex-1 text-center py-[5px] bg-secondary text-foreground text-[13px] font-semibold rounded-[6px] cursor-default shadow-sm border border-border/40">
          Order
        </div>
        <div className="flex-1 text-center py-[5px] text-muted-foreground hover:bg-secondary/50 text-[13px] font-medium rounded-[6px] cursor-pointer transition-colors border border-transparent">
          DOM
        </div>
      </div>
      ...
      <div className="p-4 flex-1 overflow-y-auto pb-[90px] custom-scrollbar">
        ...
        <div className="flex rounded-[8px] overflow-hidden bg-secondary border border-border/50 shadow-sm mb-5 relative h-[64px]">
          <button
            ...
            onClick={() => {
              setSide("Short");
              if (orderType !== "Market") form.setValue("entryPrice", Number(sellPriceText));
            }}
          ...
          <button
            ...
            onClick={() => {
              setSide("Long");
              if (orderType !== "Market") form.setValue("entryPrice", Number(buyPriceText));
            }}
          ...
          {["Market", "Limit", "Stop"].map((type) => (
            <button
              ...
              onClick={() => {
                ...
                if (type !== "Market") {
                  form.setValue("entryPrice", side === "Long" ? Number(buyPriceText) : Number(sellPriceText));
                }
              }}
        ...
        <Form {...form}>
          <form className="space-y-4 pb-2">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-1">
                  <FormLabel className="text-[13px] text-muted-foreground font-normal">
                    Price
                  </FormLabel>
                  <FormControl>
                    <div className="relative flex items-center border border-border/60 rounded-[6px] bg-card overflow-hidden focus-within:ring-1 ring-primary transition-all">
                      <Input
                        ...
                        value={
                          orderType === "Market" ? (side === "Long" ? buyPriceText : sellPriceText) : field.value
                        }
                      ...
                      <div className="absolute right-0 flex items-center h-full px-3 gap-2 bg-card text-muted-foreground">
                        ...
                        <span className="text-[13px] w-6 text-right font-medium">
                          {side === "Long" ? "Ask" : "Bid"}
                        </span>
                      </div>
                    </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-1">
                  <FormLabel className="text-[13px] text-muted-foreground font-normal flex items-center">
                  ...
                  <FormControl>
                    <div className="relative flex items-center border border-border/60 rounded-[6px] bg-card overflow-hidden focus-within:ring-1 ring-primary transition-all">
                      ...
                      <div className="absolute right-0 flex items-center h-full px-3 gap-2 bg-card text-muted-foreground">
                        <Repeat className="h-3 w-3 cursor-pointer hover:text-foreground transition-colors" />
                        <span className="text-[13px] text-foreground min-w-[50px] text-right flex items-center gap-1 font-medium">
            ...
            <Collapsible
              ...
              className="space-y-3 pt-4 border-t border-border/50 mt-6"
            >
              ...
              <CollapsibleContent className="space-y-3">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[13px] text-muted-foreground flex items-center gap-1 cursor-pointer">
                      Take profit, price <ChevronDown className="h-3 w-3" />
                    </span>
                    <Switch
                      checked={enableTp}
                        ...
                        <FormControl>
                          <div className="relative flex items-center border border-border/60 rounded-[6px] bg-card overflow-hidden focus-within:ring-1 ring-primary transition-all">
                            <Input
                              type="number"
                              step="0.001"
                              className="h-[34px] border-0 bg-transparent text-[13px] pl-3 pr-[90px] focus-visible:ring-0 focus-visible:ring-offset-0 tabular-nums"
                              {...field}
                              onChange={(e) => field.onChange(e.target.value)}
                            />
                            <div className="absolute right-0 flex items-center h-full px-3 gap-2 bg-card text-muted-foreground">
                              <Repeat className="h-3 w-3 cursor-pointer hover:text-foreground transition-colors" />
                              <span className="text-[13px] min-w-[40px] text-right text-foreground flex items-center gap-1 font-medium">
                                {tpTicks} ticks{" "}
                                <ChevronDown className="h-3 w-3 opacity-70" />
                              </span>
                            </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[13px] text-muted-foreground flex items-center gap-1 cursor-pointer">
                      Stop loss, price <ChevronDown className="h-3 w-3" />
                    </span>
                    <Switch
                      checked={enableSl}
            ...
            <div className="pt-4 border-t border-border/50">
              <span className="font-semibold text-[14px] mb-2.5 block">
                Extra settings
              </span>
              ...
              <div className="space-y-2 text-[13px]">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-muted-foreground flex items-center gap-1.5">
                    Margin{" "}
                    <Info className="h-[13px] w-[13px] text-muted-foreground/70" />
                  </span>
                  <span className="font-bold tabular-nums">
                    {margin.toFixed(2)} / {balance.toFixed(2)}
                  </span>
                </div>
                <div className="px-0.5 py-1 mb-2">
                  <Slider
                    ...
                    max={10}
                    step={1}
                    className="[&_[role=slider]]:h-[14px] [&_[role=slider]]:w-[14px] [&_[role=slider]]:border-[1px] [&_[role=slider]]:border-background [&_[role=slider]]:shadow-sm [&_[role=slider]]:bg-primary"
                  />
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold tabular-nums">
                    0.001{" "}
                    <span className="font-normal text-[11px] text-muted-foreground ml-0.5">
                      USD
                    </span>
                  </span>
                </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 pt-3 bg-card border-t border-border/50 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
        ...
        >
          <span className="font-bold text-[16px] tracking-wide mb-0.5">
            {side === "Long" ? "Buy" : "Sell"}
          </span>
          <span className="text-[11px] font-medium opacity-90 uppercase tracking-wider">
            {units} {sessionAsset} @ {price ? Number(price).toFixed(3) : 0}{" "}
            {orderType}
          </span>
        </Button>
      </div>
